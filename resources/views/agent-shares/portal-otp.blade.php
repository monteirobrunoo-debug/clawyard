...
<div class="card">
    ...
    <div class="client">Portal privado para <strong>{{ $client_name }}</strong></div>
    ...
    @if(empty($otp_sent))
        ...
        <div class="sub">
            Introduz o email <strong>registado</strong> para receberes um código único que desbloqueia <strong>todos os agentes</strong> deste portal.
        </div>
        ...
        <form method="POST" action="/p/{{ $portal_token }}/otp/request">
            @csrf
            <input type="email" name="email" placeholder="o-teu-email@empresa.pt" autocomplete="email" required autofocus inputmode="email" autocapitalize="off" autocorrect="off" spellcheck="false">
    ...
    @else
        ...
        <div class="sub">
            Verifica a tua caixa (inclui spam). Código de <strong>6 dígitos</strong>, válido 10 min.
        </div>
        ...
        <form method="POST" action="/p/{{ $portal_token }}/otp/verify">
            @csrf
        ...
        <div class="muted">
            Não recebeste? <a href="/p/{{ $portal_token }}">Pedir novo código</a>
        </div>
    @endif
</div>
</body>
